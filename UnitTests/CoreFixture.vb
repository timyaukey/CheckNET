...
Public Class CoreFixture
    ...
    Public Sub TestSecurity()
        ...
        objSec.CreateEmpty()
        gUTAssert(objSec.LoginName = "", "Login name was not empty")
        objSec.CreateUser("admin", "Administrator")
        gUTAssert(objSec.HaveUser, "Create did not remember user")
        ...
        objSec = New Security(mobjCompany)
        objSec.Load()
        gUTAssert(objSec.LoginName = "", "Login name was not empty 2")
        gUTAssert(Not objSec.FindUser("nouser"), "Should not have found user")
        gUTAssert(objSec.FindUser("admin"), "Did not find admin user")
        gUTAssert(objSec.HaveUser, "Did not remember user was found")
        gUTAssert(objSec.IsUserSignatureValid, "Admin signature invalid")
        gUTAssert(Not objSec.PasswordMatches("wrongpass"), "Password failure")
        gUTAssert(objSec.PasswordMatches("master"), "Password failure 2")

        objSec.CreateUser("john", "John Public")
        gUTAssert(objSec.LoginName = "john", "Did not remember login name")
        ...
        objSec = New Security(mobjCompany)
        objSec.Load()
        gUTAssert(objSec.FindUser("admin"), "Did not find admin 2")
        gUTAssert(objSec.LoginName = "admin", "Login was not admin 2")
        gUTAssert(Not objSec.PasswordMatches("wrongpass"), "Admin pass fail 2")
        gUTAssert(objSec.PasswordMatches("master"), "Admin pass okay 2")
        gUTAssert(objSec.IsUserSignatureValid, "Admin sig bad 2")
        gUTAssert(objSec.FindUser("john"), "Did not find john")
        gUTAssert(objSec.LoginName = "john", "Login was not john 2")
        gUTAssert(objSec.PasswordMatches("password"), "Bad john password")
        gUTAssert(objSec.IsUserSignatureValid, "Bad john sig 2")
        objSec.DbgUserXmlElement.SetAttribute("name", "new name")
        gUTAssert(Not objSec.IsUserSignatureValid, "Changing name did not change signature")
