...
Public Class UTRegister
    ...
    'Selected fields from one BaseTrx expected to be in the Register.
    ...
    Private mobjTrxReported As BaseTrx
    ...
    'Add a normal BaseTrx to Register and UTTrx array without any unit testing assertions.
    'Used to add BaseTrx which will be input to later tests. Does NOT fire any Register events.

    Public Sub LoadNormal(ByVal strNumber As String, ByVal datDate As Date, ByVal curAmount As Decimal,
                          Optional ByVal lngStatus As BaseTrx.TrxStatus = BaseTrx.TrxStatus.Unreconciled,
                          ...
                          Optional ByVal strDescr As String = "descr")

        Dim objTrx As BankTrx

        objTrx = New BankTrx(objReg)
        objTrx.NewStartNormal(True, strNumber, datDate, strDescr, "memo", lngStatus, blnFake, curNormalMatchRange, blnAwaitingReview, blnAutoGenerated, intRepeatSeq, strImportKey, strRepeatKey)
        objTrx.AddSplit("smemo", strCategoryKey, strPONumber, strInvoiceNum, datInvoiceDate, datDueDate, strTerms, strBudgetKey, curAmount)
        If Not XMLMisc.IsAttributeMissing(vcurAmount2) Then
    ...
    'Add a budget BaseTrx to Register and UTTrx array without any unit testing assertions.
    'Used to add BaseTrx which will be input to later tests. Does NOT fire any Register events.

    Public Sub LoadBudget(ByVal datDate As Date, ByVal curBudgetLimit As Decimal, ByVal datBudgetStarts As Date,
                          ByVal strBudgetKey As String, Optional ByVal blnAwaitingReview As Boolean = False,
                          Optional ByVal blnAutoGenerated As Boolean = False, Optional ByVal strRepeatKey As String = "",
                          Optional ByVal intRepeatSeq As Short = 0, Optional ByVal strDescr As String = "descr")

        Dim objTrx As BudgetTrx

        objTrx = New BudgetTrx(objReg)
        objTrx.NewStartBudget(True, datDate, strDescr, "memo", blnAwaitingReview, blnAutoGenerated, intRepeatSeq, strRepeatKey, curBudgetLimit, datBudgetStarts, strBudgetKey)
        objReg.NewLoadEnd(objTrx)
        AddTrx(objTrx)

    End Sub

    'Add a normal BaseTrx to Register and UTTrx array, with unit testing assertions.
    ...
    'Update a normal BaseTrx, with unit testing assertions.
    'Fires Register events.
    ...
    Public Sub UpdateNormal(ByVal strNumber As String, ByVal datDate As Date, ByVal curAmount As Decimal, ByVal strFailMsg As String,
                            ...
                            ByVal lngBalanceChangeLast As Integer, Optional ByVal lngStatus As BaseTrx.TrxStatus = BaseTrx.TrxStatus.Unreconciled,
                            Optional ByVal blnFake As Boolean = False, Optional ByVal curNormalMatchRange As Decimal = 0,
                            Optional ByVal strCategoryKey As String = "cat1", Optional ByVal strBudgetKey As String = "",
                            Optional ByVal vcurAmount2 As Object = Nothing, Optional ByVal strBudgetKey2 As String = "",
                            Optional ByVal blnAwaitingReview As Boolean = False, Optional ByVal blnAutoGenerated As Boolean = False,
        ...
        ClearEventReporting()
        Dim objTrxManager As NormalTrxManager = New NormalTrxManager(objReg.GetBankTrx(lngOldIndex))
        objTrxManager.Update(
            Sub(ByVal objTrx As BankTrx)
    ...
    'Add a budget BaseTrx to Register and UTTrx array, with unit testing assertions.
    'Fires Register events.
    ...
    'Update a budget BaseTrx, with unit testing assertions.
    'Fires Register events.
    ...
    Public Sub UpdateBudget(ByVal datDate As Date, ByVal curBudgetLimit As Object, ByVal datBudgetEnds As Object, ByVal strBudgetKey As String, ByVal strFailMsg As String, ByVal lngExpectedOldIndex As Integer, ByVal lngExpectedNewIndex As Integer, ByVal lngBalanceChangeFirst As Integer, ByVal lngBalanceChangeLast As Integer, Optional ByVal blnAwaitingReview As Boolean = False, Optional ByVal blnAutoGenerated As Boolean = False, Optional ByVal strRepeatKey As String = "", Optional ByVal intRepeatSeq As Short = 0)
        ...
        ClearEventReporting()
        Dim objTrxManager As BudgetTrxManager = New BudgetTrxManager(objReg.GetBudgetTrx(lngExpectedOldIndex))
        objTrxManager.Update(
            Sub(ByVal objTrx As BudgetTrx)
    ...
    'Delete a BaseTrx, with unit testing assertions.
    'Fires Register events.
    ...
    Public Sub DeleteEntry(ByVal lngIndex As Integer, ByVal blnExpectedBalanceChangedFired As Boolean, ByVal strFailMsg As String)
        ...
        ClearEventReporting()
        objReg.Delete(mobjReg.GetTrx(lngIndex), New LogDelete, "UTDeleteEntry")
    ...
    'Add a BaseTrx to the UTTrx array.

    Public Sub AddTrx(ByVal objTrx As BaseTrx)
        intTrx = intTrx + 1
    ...
    End Sub
    'Assert that each BaseTrx in the Register matches a specified element in the UTTrx array,
    ...
    Public Sub Validate(ByVal strSubTest As String, ByVal ParamArray avntUTIdx() As Object)
        ...
        If objReg.TrxCount <> (UBound(avntUTIdx) - LBound(avntUTIdx) + 1) Then
            gUTFailure("Wrong Trx count")
            Exit Sub
        End If

        For intRegIndex = 1 To objReg.TrxCount
        ...
        If mcolErrors.Count() > 0 Then
            gUTFailure("Unexpected validation errors: " & Utilities.GetFirstElement(mcolErrors))
        End If
