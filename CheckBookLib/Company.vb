...
Public Class Company
    ...
    Public Sub SaveChangedAccounts()
        Dim objAccount As Account
        Dim strBackupFile As String
        For Each objAccount In colAccounts
            If objAccount.blnUnsavedChanges Then
                strBackupFile = strBackupPath() & "\" & objAccount.strFileNameRoot & "." & Now.ToString("MM$dd$yy$hh$mm")
                If Len(Dir(strBackupFile)) > 0 Then
                    Kill(strBackupFile)
                End If
                Rename(strAccountPath() & "\" & objAccount.strFileNameRoot, strBackupFile)
                PurgeAccountBackups(objAccount)
                objAccount.Save(objAccount.strFileNameRoot)
                RaiseEvent SavedAccount(objAccount.strTitle)
            End If
        Next objAccount
    End Sub
