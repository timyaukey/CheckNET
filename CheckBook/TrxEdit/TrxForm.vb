...
Friend Class TrxForm
    ...
    Public Sub SaveNormal()
        ...
        If mblnEditMode Then
            ...
            objTrx = DirectCast(objTrxManager.objTrx, NormalTrx)
            AddSplits(objTrx)
            objTrxManager.UpdateEnd(New LogChange, mstrLogTitle)
        Else
            objTrx = DirectCast(objNewStartNormal(), NormalTrx)
            AddSplits(objTrx)
    ...
    Private Function objUpdateStartNormal() As TrxManager
        Dim objTrxManager As TrxManager = mobjReg.objGetTrxManager(mlngIndex)

        objTrxManager.UpdateStart()
        DirectCast(objTrxManager.objTrx, NormalTrx).UpdateStartNormal(txtNumber.Text, CDate(txtDate.Text), txtDescription.Text, txtMemo.Text, lngTrxStatus(),
    ...
    Private Function objCreateTrx(ByVal objCreateReg As Register) As NormalTrx
        Dim objTrx As NormalTrx
        objTrx = New NormalTrx()
        objTrx.NewStartNormal(objCreateReg, txtNumber.Text, CDate(txtDate.Text), txtDescription.Text, txtMemo.Text,
    ...
    Private Sub SaveBudget()
        ...
        If mblnEditMode Then
            objTrxManager = mobjReg.objGetTrxManager(mlngIndex)
            objTrxManager.UpdateStart()
            DirectCast(objTrxManager.objTrx, BudgetTrx).UpdateStartBudget(CDate(txtDate.Text), txtDescription.Text, txtMemo.Text, blnAwaitingReview(),
        ...
        Else
            Dim objTrx As BudgetTrx = New BudgetTrx
