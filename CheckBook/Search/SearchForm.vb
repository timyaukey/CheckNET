...
Friend Class SearchForm
    ...
    Private Sub LoadSearchInOne(ByVal strTitle As String, ByVal lngFieldID As Trx.TrxSearchField)
        With cboSearchIn
            .Items.Add(New VB6.ListBoxItem(strTitle, lngFieldID))
        End With
    End Sub
    ...
    Private Sub LoadSearchTypeOne(ByVal strTitle As String, ByVal lngTypeID As Trx.TrxSearchType)
        With cboSearchType
            .Items.Add(New VB6.ListBoxItem(strTitle, lngTypeID))
        End With
    End Sub
...
ErrorHandler:
    ...
    Private Sub cmdRecategorize_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdRecategorize.Click
        ...
        For Each objTrx In colTrx
            ...
            colSplits = objTrx.colSplits
            With objTrx
                .UpdateStartNormal(mobjReg, .strNumber, .datDate, .strDescription, .strMemo, .lngStatus, .blnFake, .curNormalMatchRange, .blnAwaitingReview, .blnAutoGenerated, .intRepeatSeq, .strImportKey, .strRepeatKey)
            End With
            For Each objSplit In colSplits
                With objSplit
        ...
        Exit Sub
ErrorHandler:
        TopError("cmdRecategorize_Click")
    End Sub
        ...
        For Each objItem In lvwMatches.Items
            If objItem.Checked Then
                ...
                If objNewTrx Is Nothing Then
                    objNewTrx = New Trx
                    datToday = Today
                    objNewTrx.NewStartNormal(mobjReg, "", datToday, objOldTrx.strDescription, objOldTrx.strMemo, Trx.TrxStatus.glngTRXSTS_UNREC, False, 0, False, False, 0, "", "")
                End If
...
ErrorHandler:
    ...
    Private Sub cmdMove_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdMove.Click
        ...
        Dim colCheckedTrx As Collection
        Dim objSelectedTrx As Trx
        ...
        For Each objTrxSrc In colTrx
            ...
            With objTrxSrc
                If objNewReg Is Nothing Then
                    ...
                    .UpdateStartNormal(mobjReg, .strNumber, datNewDate, .strDescription, .strMemo, .lngStatus, .blnFake, .curNormalMatchRange, .blnAwaitingReview, .blnAutoGenerated, .intRepeatSeq, .strImportKey, .strRepeatKey)
                    'Add the splits back.
                    For Each objSplit In colSplits
                ...
                Else
                    ...
                    objTrxNew = New Trx
                    objTrxNew.NewStartNormal(objNewReg, .strNumber, datNewDate, .strDescription, .strMemo, .lngStatus, .blnFake, .curNormalMatchRange, .blnAwaitingReview, .blnAutoGenerated, .intRepeatSeq, .strImportKey, .strRepeatKey)
...
ErrorHandler:
    ...
    Private Sub TopError(ByVal strRoutine As String)
        gTopErrorTrap("SearchForm." & strRoutine)
    End Sub

    Private Sub NestedError(ByVal strRoutine As String)
        gNestedErrorTrap("SearchForm." & strRoutine)
    End Sub
End Class